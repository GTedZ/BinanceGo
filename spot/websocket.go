...
func (s *websocket[T]) buildUrl() string {
	...
	fullUrl := s.baseUrl + "/stream?streams=" + strings.Join(streams, "/")
...
func newWebsocket[T any](baseUrl string, streams []string, onMessage func(data T), logger logging.Logger) (*websocket[T], Error) {
	socket := &websocket[T]{
		baseUrl: baseUrl,
